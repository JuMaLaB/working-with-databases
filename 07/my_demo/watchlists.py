...
# Watchlist is alos a subclass of mongoengine Document base class
class Watchlist(Document):
    name = fields.StringField(max_length=256)
    # metada will be embedded into document
    metadata = fields.EmbeddedDocumentField(WatchlistMetadata)
    # coins will be embedded into document list
...
def _seed_data():
    ...
    watchlist_data = [
        ("Bulls", "Coins to buy", "USD", [
            ("bitcoin", "Bitcoin is number one!"),
            ("ethereum", "Ethereum is number two!")
        ]),
...
def view_watchlist():
    ...
    for index, coin in enumerate(coins):
        try:
            ...
            msg = '>> KeyError key: ' + coin + " doesn't exit on CoinGecko"
